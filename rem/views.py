...
def select_rem_list(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            filt['status'] = 'U'
            filter_args = {k:v for k,v in filt.items() if v is not None}
            rem_list = Payment.objects.filter(**filter_args).exclude(requestpay__remittance__exchange__name__in=['SWIFT','CASH DEPOSIT','FDD/RTGS DEPOSIT']).order_by('requestpay__remittance__exchange','-dateresolved','requestpay__remittance__branch__code') 
            if rem_list:
        ...
        if form.is_valid():
            ...
            if settlement_type=='remittance':
                ...
                for item in q:
                    ...
                    if item_rtable:
                        item_rtable.save()
                        messages.success(request, ref+" Cash Incentive Successfully Settled in remittance table. date "+str(item_rtable.date_cash_incentive_settlement))
                    else:
                        messages.error(request, ref+' Cash Incentive Cannot be settled in remittance table')
                    
                    if item_cashin:
                        item_cashin.save()
                        messages.success(request, ref+" Cash Incentive Successfully Settled in Cash inentive table. date"+str(item_cashin.date_cash_incentive_settlement))
                    else:
...
class RemmitInfoUpdate(PermissionRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def get_form_kwargs(self):
        ...
        kw['request'] = self.request # the trick!
        kw['receiver'] = self.object.receiver
        return kw
...
def download_voucher(request, pk):
    ...
    #cash_incentive_taka, cash_incentive_ps = (str(int(rem.get_paid_cash_incentives().cash_incentive_amount//1)), "0"+str(int(Decimal(rem.get_paid_cash_incentives().cash_incentive_amount%1)*100)) if int(Decimal(rem.get_paid_cash_incentives().cash_incentive_amount%1)*100)<10 else str(int(Decimal(rem.get_paid_cash_incentives().cash_incentive_amount%1)*100))) if not rem.check_if_cash_incentive_paid() else (0,0)
    cash_incentive_taka = '0'
    cash_incentive_ps = '0'
    if rem.get_paid_cash_incentives():
        cash_incentive_taka = rem.get_paid_cash_incentives().get_decimal_left()
        cash_incentive_ps = rem.get_paid_cash_incentives().get_decimal_right()
